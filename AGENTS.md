...
- Writing section: MDX blog posts in src/content/writing/, rendered via BlogPost.astro layout with hand-written .prose CSS (not @tailwindcss/typography), max-width 720px centered; Tailwind CSS v4 preflight resets `list-style-type: none` so `.prose ul`/`.prose ol` need explicit `list-style-type: disc`/`decimal` to restore bullets and numbering
...
- Mermaid in MDX must use chart prop (`<Mermaid chart={`...`} />`) because MDX wraps slot content in `<p>` tags; Mermaid@11 doesn't support OKLCH colors — `BlogPost.astro` has a `resolveColor()` canvas helper that converts to sRGB hex at runtime; subgraph titles need explicit `titleColor`, `clusterBkg`, `clusterBorder` theme variables for readability; `Note over` syntax only accepts two participants (three causes syntax errors); self-reference arrows (`A-->>A:`) are unreliable — use `Note over A:` instead; `<br>` tags work inside JSX template literal chart props without MDX parsing issues
- Mermaid `stateDiagram-v2` notes: `\n` inside template literal note text (e.g. `note right of State: line1\nline2`) causes Mermaid to parse subsequent lines as separate state definitions, creating ghost boxes — keep all `note right of` content on a single line
...
- In Slidev markdown, HTML comments (`<!-- -->`) inside frontmatter blocks (between `---` delimiters) are parsed as invalid YAML attributes, crashing Vue rendering — use YAML `#` comments instead
- Slidev PDF export (`slidev export`) requires `--wait 15000 --wait-until networkidle` to avoid blank pages — async content (Vue components, Mermaid) needs rendering time before Playwright captures the print page
- Cloudflare Pages only processes `_redirects` at the deployment root (`dist/`), not in subdirectories — nested `_redirects` files generated by Slidev builds are silently ignored
